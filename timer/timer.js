...
audiofile.volume = slider_volume.value;
slider_volume.addEventListener("input", e => {
    audiofile.volume = slider_volume.value;
    document.getElementById("advoldis").innerHTML=(audiofile.volume*100).toFixed();
  });
  ...
  window.onload = function() {
	var obj = document.getElementsByClassName('settext');
	if (!obj) return;
	for (var i = 0; i < obj.length; i++) {
			if (obj[i].type != "text") continue;
			obj[i].onfocus = value_clear;
			obj[i].onblur = value_default;
	}
}
